...
import (
	...
	plgr "github.com/ZihuaZhang/fabric-protos-go/ledger/queryresult"
	...
	aclmocks "github.com/ZihuaZhang/fabric/core/aclmgmt/mocks"
	...
	ledgermock "github.com/ZihuaZhang/fabric/core/ledger/mock"
...
func endTxSimulation(peerInstance *peer.Peer, channelID string, ccid *pb.ChaincodeID, txsim ledger.TxSimulator, _ []byte, commit bool, prop *pb.Proposal, blockNumber uint64) error {
	...
	if lgr := peerInstance.GetLedger(channelID); lgr != nil {
		if commit {
			...
			// get the envelope
			env, err := protoutil.CreateSignedTx(nil, prop, signer, resp)
			if err != nil {
				return err
			}
