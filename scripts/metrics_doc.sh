...
generate_doc() {
    local gendoc_command="go run github.com/ZihuaZhang/fabric/common/metrics/cmd/gendoc"
    local orderer_prom
    ...
    local orderer_deps=()
    while IFS= read -r pkg; do orderer_deps+=("$pkg"); done < <(go list -deps github.com/ZihuaZhang/fabric/cmd/orderer | sort -u | grep -E "(hyperledger|SmartBFT-Go/consensus)")
    ...
    local peer_deps=()
    while IFS= read -r pkg; do peer_deps+=("$pkg"); done < <(go list -deps github.com/ZihuaZhang/fabric/cmd/peer | sort -u | grep hyperledger)
